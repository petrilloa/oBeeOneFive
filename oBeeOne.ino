...
void FireBaseHandler(const char *event, const char *data) {
  ...
  JsonObject & oBeeJS = root["oBee"];  // query root
  if (!oBeeJS.success()) {
...
void Publish()
{
    if(ms - msLast > publishTime)
    {
        ...
        //root["status"] = 1;
        root["oBeeId"] = oBeeID.c_str();
        root["losantId"] = losantId.c_str();
